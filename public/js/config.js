app.config = {

  server: {
    baseUrl: '#',
    defaultIp: '127.0.0.1',
    defaultPort: '9600'
  },
  works: {
    gzhacker: [
      {
        isFirst: true,
        imgUrl: './public/images/hacker01.JPG',
        imgAlt: '2',
        href: 'https://plus.google.com/photos/114920001331857622210/albums/5830155029390732513?authkey=CNPT-Y-V3fqJ4gE',
        caption: '人生中见到第一批程序员到此时着实除了一个android加减乘除计算器和js计时器便没有任何成果了，也敢在大牛云集的bata提点子,点子是《手机防摔保护程序》。——第四届广州高校编程马拉松'
      },
      {
        imgUrl: './public/images/hacker02.JPG',
        imgAlt: '2',
        href: 'https://plus.google.com/photos/114920001331857622210/albums/5830155029390732513?authkey=CNPT-Y-V3fqJ4gE',
        caption: '这个位置是现场组的唯一一位队友，和我一样的编程基础，在马拉松进行了6个小时之后和我道别，我孤军挣扎了一会也只好去吃饭了。'
      },
      {
        imgUrl: './public/images/hacker03.JPG',
        imgAlt: '3',
        href: 'https://plus.google.com/photos/114920001331857622210/albums/5830155029390732513?authkey=CNPT-Y-V3fqJ4gE',
        caption: '实现自我保护功能的想法是利用手机自带的加速度传感器，计算和判断非正常移动则采取措施（最后是没有找到可行的措施），和GDG三位导师做自行车助手的想法扯上了一点关系，然后anson收留我一起写bikeplus。'
      },
      {
        imgUrl: './public/images/hacker04.JPG',
        imgAlt: '4',
        href: 'https://plus.google.com/photos/114920001331857622210/albums/5830155029390732513?authkey=CNPT-Y-V3fqJ4gE',
        caption: '没有做出什么成果，在anson的指导下完成了条形图的绘制，然后凌晨在2楼露天院子的沙发上睡了5个小时，但是第一次24编程的体会是：我将来可以成为程序员。'
      }
    ],
    codejam: [
      {
        isFirst: true,
        imgUrl: './public/images/codejam00.JPG',
        imgAlt: '1',
        href: 'http://segmentfault.com/e/codejam-guangzhou-2013-p3',
        caption: '两个人的组合（鹏飞：硬件出生的软件工程师 大宝：硬件工程师）需要一个安卓开发，没有任何准备的我有幸加入。<br>——2013中国移动开发者大赛'
      },
      {
        imgUrl: './public/images/codejam01.JPG',
        imgAlt: '1',
        href: 'http://segmentfault.com/e/codejam-guangzhou-2013-p3',
        caption: '编程进行了24个小时之后，我们《唤醒灯》成果展示时间，鹏飞向参赛者解说，我在幕后操作手机客户端，屏幕上看到的黑白时间定时界面就是我风雨不动摇的黑白风格了。'
      },
      {
        imgUrl: './public/images/codejam02.JPG',
        imgAlt: '2',
        href: 'http://segmentfault.com/e/codejam-guangzhou-2013-p3',
        caption: '灯已经慢慢亮到可以唤醒睡眠的人，大家都在等遥控关灯，结果android和单片机的连接超时已经断开，求原谅。'
      },
      {
        imgUrl: './public/images/codejam03.JPG',
        imgAlt: '3',
        href: 'http://segmentfault.com/e/codejam-guangzhou-2013-p3',
        caption: '演示出状况还是得了第四名，后来主办方计算了作品数量解释说第四名开始是优秀奖，好吧，就是第一名优秀奖。'
      }
    ],
    intern: [
      {
        isFirst: true,
        imgUrl: './public/images/intern01.jpg',
        imgAlt: '1',
        caption: '“三个月实习之后就可以拿到学分了”，离开前最真最傻的想法。'
      },
      {
        imgUrl: './public/images/intern02.jpg',
        imgAlt: '2',
        caption: '一年零三个月陪伴我所有编程之路的操作系统，求助了linux大牛也没能成功编译tplink55××驱动，解决办法是换网卡。'
      },
      {
        imgUrl: './public/images/intern03.jpg',
        imgAlt: '3',
        caption: '他们很忙我很少和他们交流，这块地方总是停电和服务器down机，以至于最初的开发过程满满的都是技术和硬件的挑战。'
      },
      {
        imgUrl: './public/images/intern04.jpg',
        imgAlt: '4',
        caption: '参加了四个线上活动的应用研发，参与Java后台又参与Javascript前端，加上没用心，我不是反应迟钝，相信我！'
      },
      {
        imgUrl: './public/images/intern05.jpg',
        imgAlt: '5',
        caption: '没有强烈留在公司的愿望成真了。<br>“推广产品和推倒妹子”一点也不好笑了。'
      },
      {
        imgUrl: './public/images/intern06.jpg',
        imgAlt: '6',
        caption: '为当初的轻浮付出的代价——回不去的位置。'
      }
    ],
    hope: [
      {
        isFirst: true,
        imgUrl: './public/images/hope01.JPG',
        imgAlt: '1',
        href: 'https://plus.google.com/photos/114920001331857622210/albums/5841418929094126033',
        caption: '不会孤立我。'
      },
      {
        imgUrl: './public/images/hope02.JPG',
        imgAlt: '1',
        caption: '背影迷人，头发修理过。'
      },
      {
        imgUrl: './public/images/hope03.jpg',
        imgAlt: '1',
        caption: '台上最闪耀，淡定不猥琐。'
      },
      {
        imgUrl: './public/images/hope04.JPG',
        imgAlt: '1',
        caption: '头发梳了，营养良。'
      },
      {
        imgUrl: './public/images/hope05.JPG',
        imgAlt: '1',
        caption: '摄像头注意到我。'
      },
      {
        imgUrl: './public/images/hope06.jpg',
        imgAlt: '1',
        caption: '数不清的朋友，4个，5个。。。。'
      },
      {
        imgUrl: './public/images/hope07.jpg',
        imgAlt: '1',
        caption: '美丽回眸，长发飘飘。。'
      },
      {
        imgUrl: './public/images/hope08.jpg',
        imgAlt: '1',
        caption: '女神的智商。'
      },
      {
        imgUrl: './public/images/hope09.jpg',
        imgAlt: '1',
        caption: '活在爱丽丝梦游仙境。'
      },
      {
        imgUrl: './public/images/hope.JPG',
        imgAlt: '1',
        caption: '父亲还在我身边'
      },
      {
        imgUrl: './public/images/hope10.jpg',
        imgAlt: '1',
        caption: '技术牛人，2个，3个屏幕'
      },
      {
        imgUrl: './public/images/hope11.jpg',
        imgAlt: '1',
        caption: '吉他厉害。'
      },
      {
        imgUrl: './public/images/hope12.jpg',
        imgAlt: '1',
        caption: 'big breasts.'
      }
    ]
  },
  ablum: [
    {
      tagId: 0,
      tagName: 'GZ第四届Hacker'
    },
    {
      tagId: 1,
      tagName: '2013移动开发者',
    },
    {
      tagId: 2,
      tagName: 'UC暑假',
      select: true
    },
    {
      tagId: 3,
      tagName: '希望与现实'
    }
  ]
  
};